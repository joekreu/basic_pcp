...
parsers.
3. Better understand the meaning of _precedence correct_ parsing.
...
`pcp_rec_0_0`, `pcp_rec_0_1`, `pcp_rec_0_2` and `pcp_rec_03` are recursive and
more or less _functional_ (in the sense of functional programming).
...
The output can be more verbose or more concise. Use the option `-h` (with any
basic parser module) to find out all ways to run the parsers:
...
This project was inspired by works of _Thoedore Norvell_, _Aleksey Kladov_
(_matklad_), _Andy Chu_, _Eli Bendersky_, _Jean-Marc Bourguet_,
_Fredrik Lundh_ (_effbot_), _Bob Nystrom_ and others.

The _correctness test_ and _operator ranges_ (functions `_is_prec_correct`,
`_lrange`, `_rrange` in `helpers.py`) are adaptions of definitions by
_Annika Aasa_ in
...
See the text [Detailed Guide](DETAILED_GUIDE.md) in this repo.