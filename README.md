...
the job of the parsers. `$BEGIN` and `$END` act as a kind of _operators_ in
the process of parsing. A negative _rbp_ is assigned to `$BEGIN`, and a
negative _lbp_ to `$END`.
...
where `PARSER_MODULE` is one of the nine parser modules. Example:
